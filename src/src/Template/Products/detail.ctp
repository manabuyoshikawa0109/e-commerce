<?php
/**
 * 商品詳細。template/single-product.html を反映したもの。
 *
 * 元テンプレートのうち、対応するデータを持たない次のセクションは省いている。
 *   - フラッシュセール（終了時刻・販売済み割合のデータが無い）
 *   - 商品動画（動画URLのカラムが無い）
 *   - レビュー一覧／レビュー投稿（reviews テーブルが無い）
 * カラー・サイズの選択は商品情報に持っていないため、見た目のみ残している。
 *
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Product $product
 * @var \App\Model\Entity\Product[]|\Cake\Datasource\ResultSetInterface $relatedProducts
 */
...
?>
<div class="back-button"><a href="<?= $this->Url->build('/') ?>"><i class="ti ti-arrow-left"></i></a></div>
<?php
$this->end();
...
<div class="container p-0">
  ...
  <div class="product-description pb-3">
    ...
    <div class="product-title-meta-data bg-white mb-3 py-3">
      <div class="container d-flex justify-content-between mb-2">
        <div class="p-title-price">
          <h5 class="mb-1"><?= h($product->name) ?></h5>
          <p class="sale-price mb-0 lh-1"><?= $this->Number->currency($product->price, 'JPY') ?></p>
        </div>
        <div class="p-wishlist-share"><a href="#"><i class="ti ti-heart"></i></a></div>
      </div>
      <!-- Ratings-->
      <div class="product-ratings">
        <div class="container d-flex align-items-center justify-content-between">
          <div class="ratings d-flex align-items-center gap-1"><i class="ti ti-star-filled"></i><i class="ti ti-star-filled"></i><i class="ti ti-star-filled"></i><i class="ti ti-star-filled"></i><i class="ti ti-star-filled"></i><span>3 ratings</span></div>
            ...
            <div class="card user-data-card">
              <div class="card-body p-0">
                <div class="single-profile-data d-flex align-items-center justify-content-between">
                  ...
                  <div class="data-content"><?= h($product->category_label) ?></div>
                </div>
                <div class="single-profile-data d-flex align-items-center justify-content-between">
                  <div class="title d-flex align-items-center">
                  ...
                  <div class="data-content"><?= h($product->brand_name) ?></div>
                </div>
                <div class="single-profile-data d-flex align-items-center justify-content-between">
                  <div class="title d-flex align-items-center">
                  ...
                  <div class="data-content"><?= h($product->condition_label) ?></div>
                </div>
              </div>
            </div>
          </div>
    ...
    <div class="cart-form-wrapper bg-white mb-3 py-3">
      <div class="container">
        <?php if ($product->is_sold_out) : ?>
        <button class="btn btn-secondary w-100" type="button">売り切れのため購入できません</button>
        <?php else : ?>
        <button class="btn btn-primary w-100" type="button">商品を購入する</button>
        <?php endif; ?>
      </div>
    </div>
  </div>
